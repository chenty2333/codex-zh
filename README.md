...
**中文仅用于当前显示，不建立译文历史，也不将译文写入磁盘。** 原生会话保存实际提交的英文输入和 Codex 原始回复；`resume` 显示原始历史，之后的新消息继续翻译。
...
- **一个回复条目只形成一份确定译文。显示的所有译文增量拼接起来，就是最终回复；不会在结束时重新翻译。** 这份文字只临时保留到当前回合的最终快照发送完成，随后释放。
...
| 中文用户消息回显 | 当前回合显示原始中文；Codex 服务端保存的上下文是译后的英文 |
| 回复正文与计划条目 | 翻译为中文，增量和最终文字一致 |
| 菜单、工具日志、推理摘要、审批问题 | 保留原生文字，可能仍为英文 |
| `resume` / `fork` / 历史读取 | 直接显示原生历史，不翻译、不恢复旧中文；新消息继续双向翻译 |
| 输入回溯历史 | 本启动器对 TUI 和后台设置 `history.persistence="none"`，避免原生输入历史文件另存中文 prompt |
...
重新打开或重新加载会话历史时，旧文字会显示原文。当前屏幕上的中文由原生 TUI 管理；代理不会为日后恢复保留一份副本。同样的文字在新的消息中出现时，会重新翻译。

## 内存与超限行为

代理不保存随会话长度累积的历史。中文输入回显数据和确定译文只保留到当前回合完成，切换/取消订阅会话、关闭连接时也会释放。

默认单条待译文本上限为 128 Ki 个 UTF-16 字符单位，代理保留文本的总预算为 4 Mi 个字符单位，最多 256 个当前条目。容量在接收翻译工作前预留，译文过度膨胀时会拒绝该批次并保留原文。一个长回合达到条目或总预算上限后，新增回复按原生方式显示原文；过大的输入会在提交前报错，避免悄悄绕过翻译。

对超大输出，代理会完整转发已经暂存的原文前缀，然后按原生方式继续显示后续原文，不积攒整条消息。若只有最终条目才超限，则完整显示最终原文。超限分支不产生译文，流式/最终源文字的处理交回原生 TUI。

传输层另有限制待处理帧和发送缓冲的背压，避免翻译较慢时无限排队。这些限制针对代理的额外数据；Node.js 运行时、临时解析对象、原生 Codex/TUI 的上下文和屏幕历史有各自的内存开销，因此 4 Mi 字符不是整个进程 RSS 上限。
...
| `CODEX_ZH_MAX_TEXT_CHARS` | `131072`，单条待译文本/输入引用信息大小上限 |
| `CODEX_ZH_MAX_BUFFERED_CHARS` | `4194304`，保留文本的总字符预算；至少为单条上限的四倍 |
| `CODEX_ZH_MAX_LIVE_ITEMS` | `256`，待处理输入和当前回合文字条目的总数上限 |
| `CODEX_ZH_CODEX_BIN` | `codex`，原生可执行文件 |
| `CODEX_ZH_PASSTHROUGH` | `1` 时关闭翻译 |

代理不创建译文状态目录，不读写原文/译文映射，也不记录 API 密钥、请求正文或模型响应正文到日志。旧版 `CODEX_ZH_STATE_DIR` 已不再使用。原生 Codex 仍照常保存英文输入与原始回复，用于恢复上下文；保护片段中的原始中文代码/字面量仍原样保留。非保护的自然语言会发送给 DeepSeek，这是翻译所必需的。
...
`test:native` 会实际重启 app-server 并恢复测试会话，确认旧记录英文、新消息中文；`test:tui` 通过 Python 3 的伪终端驱动真实 TUI，模型和翻译 API 均为本地确定性模拟；`test:memory` 用 20,000 个不同的合成回合检查释放和 GC 后的堆占用；`test:live` 使用真实 DeepSeek API 和合成文本，会消耗少量 API 额度。合成测试报告在被 Git 忽略的 `.test-state/` 中，运行时不会记录用户对话译文。
